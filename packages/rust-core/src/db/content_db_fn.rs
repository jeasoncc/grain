...
pub async fn update(
    ...
    let existing = find_by_node_id(db, node_id)
        .await?
        .ok_or_else(|| AppError::not_found(format!("Content for node {}", node_id)))?;
...
pub async fn delete(db: &DatabaseConnection, id: &str) -> AppResult<()> {
    ...
    if result.rows_affected == 0 {
        return Err(AppError::not_found(format!("Content {}", id)));
    }
...
mod tests {
    ...
    async fn test_delete_content() {
        ...
        create(&db, "content-1".to_string(), node_id, "内容".to_string())
            .await
            .unwrap();
