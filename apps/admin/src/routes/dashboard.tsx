...
import { Users, Globe, Activity, Calendar, TrendingUp, Zap } from 'lucide-react';
...
function DashboardPage() {
  ...
  const growthRate = stats.total > 0 
    ? ((stats.today / stats.total) * 100).toFixed(1)
    : '0';
              ...
              return (
                ...
                >
                  ...
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4 relative z-10">
                    ...
                    <div className={`rounded-xl p-3 ${stat.iconBg} shadow-lg group-hover:scale-110 transition-transform duration-300`}>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">今日活跃度</span>
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-24 bg-muted rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transition-all duration-500"
                        style={{ width: `${Math.min((stats.today / Math.max(stats.thisWeek, 1)) * 100, 100)}%` }}
                      />
                    </div>
                    <span className="text-sm font-medium">
                      {stats.thisWeek > 0 ? Math.round((stats.today / stats.thisWeek) * 100) : 0}%
                    </span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">本周占比</span>
                  <span className="text-sm font-medium">
                    {stats.total > 0 ? ((stats.thisWeek / stats.total) * 100).toFixed(1) : 0}%
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">本月占比</span>
                  <span className="text-sm font-medium">
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">独立IP占比</span>
                  <span className="text-sm font-medium">
                    {stats.total > 0 ? ((stats.uniqueIPs / stats.total) * 100).toFixed(1) : 0}%
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">平均访问/日</span>
                  <span className="text-sm font-medium">
                    {stats.thisWeek > 0 ? (stats.thisWeek / 7).toFixed(1) : 0}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">今日独立IP</span>
