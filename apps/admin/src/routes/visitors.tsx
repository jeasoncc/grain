  ...
  columnHelper.accessor('visitedAt', {
    ...
    cell: (info) => (
      <div className="flex flex-col">
        <span className="text-sm font-medium">{formatDate(info.getValue())}</span>
...
function VisitorsPage() {
  ...
  if (filterBrowser !== 'all') {
    filteredVisitors = filteredVisitors.filter(v => 
      v.browser?.toLowerCase() === filterBrowser.toLowerCase()
    );
  }
  if (filterDevice !== 'all') {
    filteredVisitors = filteredVisitors.filter(v => {
      if (filterDevice === 'mobile') {
        return v.device?.toLowerCase().includes('mobile') || 
  ...
  const browsers = Array.from(new Set(visitors.map(v => v.browser).filter(Boolean)));
  const devices = Array.from(new Set(visitors.map(v => v.device).filter(Boolean)));
            ...
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 font-mono text-sm">{selectedVisitor.ip}</div>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">访问路径</label>
                  <div className="mt-1 text-sm">{selectedVisitor.path || '/'}</div>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">浏览器</label>
                  <div className="mt-1">
                    <Badge variant="secondary">{selectedVisitor.browser || 'Unknown'}</Badge>
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    {(() => {
                      ...
                      return (
                        <>
                          <DeviceIcon className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{selectedVisitor.device || 'Unknown'}</span>
                        </>
                      );
                    })()}
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <Badge variant="outline">{selectedVisitor.os || 'Unknown'}</Badge>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">访问时间</label>
                ...
                {selectedVisitor.country && (
                  <div>
                    <label className="text-sm font-medium text-muted-foreground">国家/地区</label>
                    <div className="mt-1 text-sm">{selectedVisitor.country}</div>
                  </div>
                )}
              ...
              {selectedVisitor.referer && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">来源页面</label>
                  <div className="mt-1 text-sm break-all">{selectedVisitor.referer}</div>
                </div>
              )}
