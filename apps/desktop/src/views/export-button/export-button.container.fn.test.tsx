...
describe("ExportButtonContainer", () => {
	...
	it("should call exportProject and show success toast on successful export", async () => {
		...
		await waitFor(() => {
			expect(toast.success).toHaveBeenCalledWith("Export successful: Test Workspace.txt")
		})
	})
	...
	it("should use default workspace title when not provided", async () => {
		...
		await waitFor(() => {
			expect(toast.success).toHaveBeenCalledWith("Export successful: workspace.epub")
		})
	})
	...
	it("should pass className prop to view", () => {
		render(<ExportButtonContainer {...defaultProps} className="custom-class" />)
		const button = screen.getByRole("button", { name: /export/i })
