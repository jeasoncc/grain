...
describe("mermaid.render.fn", () => {
	...
	describe("renderMermaid", () => {
		...
		it("should use provided containerId", async () => {
			...
			expect(mermaid.render).toHaveBeenCalledWith("my-container-id", "flowchart TD\n  A --> B")
		})
		...
		it("should return friendly error for syntax error with line number", async () => {
			const mermaid = await getMermaidMock()
			mermaid.render.mockRejectedValue(new Error("Syntax error on line 3: unexpected token"))
		...
		it("should return friendly error for lexical error", async () => {
			const mermaid = await getMermaidMock()
			mermaid.render.mockRejectedValue(new Error("Lexical error: unrecognized character"))
