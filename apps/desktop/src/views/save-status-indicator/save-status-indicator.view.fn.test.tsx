...
describe("SaveStatusIndicatorView", () => {
	...
	describe("保存状态显示", () => {
		...
		it('should display "Unsaved changes" when saved but hasUnsavedChanges is true', () => {
			render(<SaveStatusIndicatorView {...defaultProps} status="saved" hasUnsavedChanges={true} />)
	...
	describe("手动保存状态", () => {
		it('should display "Saving manually..." when isManualSaving is true', () => {
			render(<SaveStatusIndicatorView {...defaultProps} isManualSaving={true} status="saved" />)

			expect(screen.getByText("Saving manually...")).toBeInTheDocument()
		})

		it("should prioritize manual saving over other statuses", () => {
			render(<SaveStatusIndicatorView {...defaultProps} isManualSaving={true} status="error" />)
	...
	describe("最后保存时间显示", () => {
		it("should not display last save time by default", () => {
			const now = Date.now()
			render(<SaveStatusIndicatorView {...defaultProps} status="saved" lastSaveTime={now} />)
	...
	describe("错误信息显示", () => {
		it("should set title attribute with error message", () => {
			...
			const { container } = render(
				<SaveStatusIndicatorView {...defaultProps} status="error" errorMessage={errorMessage} />,
			)
		...
		it("should not set title attribute when no error message", () => {
			const { container } = render(<SaveStatusIndicatorView {...defaultProps} status="error" />)
	...
	describe("图标显示", () => {
		it("should render Check icon when saved", () => {
			const { container } = render(<SaveStatusIndicatorView {...defaultProps} status="saved" />)
		...
		it("should render Loader2 icon when saving", () => {
			const { container } = render(<SaveStatusIndicatorView {...defaultProps} status="saving" />)
		...
		it("should render Save icon when unsaved", () => {
			const { container } = render(<SaveStatusIndicatorView {...defaultProps} status="unsaved" />)
