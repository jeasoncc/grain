 ...
 * 获取单个标签（不存在时抛出错误）
 */
export const getTagOrFail = (id: string): TE.TaskEither<AppError, TagInterface> =>
	pipe(
		getTag(id),
		TE.chain((tag) =>
			tag
...
export const createTag = (input: TagCreateInput): TE.TaskEither<AppError, TagInterface> =>
	pipe(TE.of(encodeCreateTag(input)), TE.chain(api.createTag), TE.map(decodeTag))
 ...
 * 增加标签使用计数
 */
export const incrementTagCount = (id: string): TE.TaskEither<AppError, TagInterface> =>
	pipe(api.incrementTagCount(id), TE.map(decodeTag))

/**
 * 减少标签使用计数
 */
export const decrementTagCount = (id: string): TE.TaskEither<AppError, TagInterface> =>
	pipe(api.decrementTagCount(id), TE.map(decodeTag))

/**
 * 删除标签
 */
export const deleteTag = (id: string): TE.TaskEither<AppError, void> => api.deleteTag(id)

/**
 * 删除工作区所有标签
 */
export const deleteTagsByWorkspace = (workspaceId: string): TE.TaskEither<AppError, number> =>
	api.deleteTagsByWorkspace(workspaceId)
...
export const rebuildTagCache = (workspaceId: string): TE.TaskEither<AppError, void> =>
	api.rebuildTagCache(workspaceId)
...
export const recalculateTagCounts = (workspaceId: string): TE.TaskEither<AppError, void> =>
	api.recalculateTagCounts(workspaceId)
