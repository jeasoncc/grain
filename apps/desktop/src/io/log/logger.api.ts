...
export const updateLogConfig = (config: Partial<LogConfig>): TE.TaskEither<AppError, void> => {
	...
	if (!validation.isValid) {
		return TE.left(logConfigError(`Invalid configuration: ${validation.errors.join(", ")}`))
	}
...
export { logDebugAsync, logInfoAsync, logSuccessAsync, logWarnAsync, logErrorAsync, logTraceAsync }
...
export { debug, info, success, warn, error, trace }
...
export const queryLogs = (
	...
	if (!advancedFeatures) {
		return TE.left(
			logConfigError("Advanced log features not available. Please inject them from flows layer."),
		)
	}
 ...
 */
export const getRecentLogs = (limit = 100, levelFilter?: readonly LogLevel[]) => {
	if (!advancedFeatures) {
		return TE.left(
			logConfigError("Advanced log features not available. Please inject them from flows layer."),
		)
 ...
 * @returns TaskEither<AppError, LogEntry[]>
 */
export const getRecentErrors = (limit: number = 20, hours: number = 24) => {
	if (!advancedFeatures) {
		return TE.left(
			logConfigError("Advanced log features not available. Please inject them from flows layer."),
		)
