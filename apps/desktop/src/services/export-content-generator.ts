/**
 * 导出内容生成器 - 从现有导出服务中提取内容生成逻辑
 * Export Content Generator - Extract content generation logic from existing export service
 */
...
/**
 * 获取项目的完整内容数据
 * Get complete project content data
 */
...
export async function generateExportBlob(
	...
	switch (format) {
		case "txt":
			return generateTxtBlob(project, chapterContents, opts);
		case "docx":
			return generateDocxBlob(project, chapterContents, opts);
		case "pdf":
...
/**
 * 生成 PDF 格式 Blob (使用打印样式)
 * Generate PDF format blob (using print styles)
 */
function generatePdfBlob(
	...
	// For PDF, we'll generate HTML content that can be used with print-to-PDF
