...
export interface NewNodeDialogProps {
  ...
  onCreateNode: (parentId: string | null, type: NodeType, title: string) => void;
  defaultType?: NodeType;
}

type CreatableNodeType = "folder" | "file" | "canvas" | "diary";

const nodeTypeConfig: Record<CreatableNodeType, { icon: typeof Folder; label: string; description: string }> = {
  folder: {
    icon: Folder,
    label: "Folder",
...
export function NewNodeDialog({
  ...
  const [selectedType, setSelectedType] = useState<CreatableNodeType>(defaultType);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>Create New {nodeTypeConfig[selectedType].label}</DialogTitle>
            <DialogDescription>
              {parentId ? "Create inside the selected folder" : "Create at root level"}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(Object.keys(nodeTypeConfig) as CreatableNodeType[]).map((type) => {
