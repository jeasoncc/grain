...
const backupMetadataGenerator = fc.record({
	...
	projectCount: fc.integer({ min: 0, max: 100 }),
	nodeCount: fc.integer({ min: 0, max: 1000 }),
	contentCount: fc.integer({ min: 0, max: 1000 }),
	tagCount: fc.integer({ min: 0, max: 50 }),
...
const backupDataGenerator = fc.record({
	...
	nodes: fc.array(fc.record({ id: fc.string(), title: fc.string(), workspace: fc.string() }), { maxLength: 100 }),
	contents: fc.array(fc.record({ id: fc.string(), nodeId: fc.string(), content: fc.string() }), { maxLength: 100 }),
	drawings: fc.array(fc.record({ id: fc.string(), data: fc.string() }), { maxLength: 10 }),
	attachments: fc.array(fc.record({ id: fc.string(), filename: fc.string() }), { maxLength: 20 }),
	tags: fc.array(fc.record({ id: fc.string(), name: fc.string() }), { maxLength: 50 }),
	dbVersions: fc.array(fc.record({ version: fc.string(), appliedAt: fc.string() }), { maxLength: 5 }),
}) as fc.Arbitrary<BackupData>

const backupInfoGenerator = fc.record({
	filename: fc.string({ minLength: 1, maxLength: 100 }).map(s => `backup_${s}.json`),
	path: fc.string({ minLength: 1, maxLength: 200 }).map(s => `/path/to/${s}`),
...
describe("Property Test: Backup Data Integrity", () => {
		...
		fc.assert(
			fc.property(
				...
				(backupData) => {
					...
					expect(new Date(backupData.metadata.timestamp).toISOString()).toBe(backupData.metadata.timestamp)
	...
	it("Property 3.3: Backup format version compatibility", () => {
		...
		fc.assert(
			fc.property(
				fc.array(localBackupRecordGenerator, { minLength: 1, maxLength: 5 }),
				async (backups) => {
					// Clear mocks before each test
					vi.clearAllMocks()
	...
	it("Property 3.5: Backup size limits and constraints", () => {
		...
		fc.assert(
			fc.property(
				backupInfoGenerator,
				fc.integer({ min: 1, max: 5 }), // maxBackups (smaller range)
				fc.array(localBackupRecordGenerator, { minLength: 0, maxLength: 3 }), // existing backups (smaller)
