...
// Batch logging
import { createBatchLogFlow, forceFlushAllLogs, getBufferStatus } from "./batch-log.flow"
// Configuration
import {
...
): TE.TaskEither<AppError, LogQueryResult> => optimizedQueryLogsFlow(options)
...
): TE.TaskEither<AppError, LogQueryResult> => paginatedQueryLogsFlow(page, pageSize, filters)
...
export const searchLogFlow = (
	...
	limit: number = 100,
): TE.TaskEither<AppError, LogQueryResult> => searchLogsFlow(searchTerm, filters, limit)
...
export const getRecentErrorsFlow = (
	limit: number = 20,
	hours: number = 24,
): TE.TaskEither<AppError, readonly LogEntry[]> => getRecentErrorLogsFlow(limit, hours)
 ...
 */
export const checkNeedsCleanupFlow = (): TE.TaskEither<AppError, boolean> => checkNeedsCleanup()
 ...
 * @returns TaskEither<AppError, void>
 */
export const flushPendingLogsFlow = (): TE.TaskEither<AppError, void> => forceFlushAllLogs()
 ...
 * @returns Task<void>
 */
export const handleLogErrorFlow = (error: AppError, fallbackMessage?: string): T.Task<void> =>
	T.fromIO(() => {
		// 降级到控制台输出
