...
// IO
import { clearOldLogsFromSQLite, getLogStatsFromSQLite } from "@/io/log/log.storage.api"
import { error, info, warn } from "@/io/log/logger.api"
...
export const getAutoCleanupConfig = (): AutoCleanupConfig =>
	getJson(STORAGE_KEYS.AUTO_CLEANUP_CONFIG, AutoCleanupConfigSchema, DEFAULT_AUTO_CLEANUP_CONFIG)
...
export const getCleanupHistory = (): CleanupHistory =>
	getJson(STORAGE_KEYS.CLEANUP_HISTORY, CleanupHistorySchema, { history: [], totalCleanups: 0 })
 ...
 * @returns void
 */
export const startAutoCleanupTimer = (config: AutoCleanupConfig = getAutoCleanupConfig()): void => {
	// 停止现有定时器
		...
		checkNeedsCleanup(config)()
			.then((result) => {
				if (result._tag === "Right" && result.right) {
					// 需要清理，执行自动清理
					executeAutoCleanup(config)().then((cleanupResult) => {
						if (cleanupResult._tag === "Right") {
							info("[AutoCleanup] Cleanup completed", {
								entriesRemoved: cleanupResult.right.entriesRemoved,
							})
						} else {
