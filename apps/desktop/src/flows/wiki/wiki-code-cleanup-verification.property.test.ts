...
describe("Property 6: Wiki Code Cleanup Consistency", () => {
	it("should verify Wiki files use SQLite APIs and not Dexie", async () => {
		await fc.assert(
			fc.asyncProperty(
				fc.constant("wiki-cleanup-check"),
				async () => {
					...
					for (const filePath of wikiFiles) {
						try {
							...
							if (content.includes('database') || content.includes('query') || content.includes('get')) {
								const usesSqlite = usesSQLiteAPIs(content)
								if (!usesSqlite && content.length > 100) { // Skip very small files
	...
	it("should verify no orphaned Wiki code exists", async () => {
		await fc.assert(
			fc.asyncProperty(
				fc.constant("orphaned-code-check"),
				async () => {
