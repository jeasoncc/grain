...
describe("Property Test: Backup Data Integrity", () => {
	...
	it("Property 3.1: Export format validation works correctly", async () => {
		...
		await fc.assert(
			fc.asyncProperty(
				fc.string({ minLength: 1, maxLength: 50 }), // projectId
				fc.string().filter(s => !["pdf", "docx", "txt", "epub"].includes(s)), // invalid format
				async (projectId, invalidFormat) => {
					const exportModule = require("./export-project.flow")
					
					// The property: invalid formats should be rejected
					await expect(exportModule.exportProject(projectId, invalidFormat)).rejects.toThrow("不支持的导出格式")
	...
	it("Property 3.2: Content extraction handles all input types", () => {
		...
		fc.assert(
			fc.property(
				fc.oneof(
					// Valid Lexical JSON
					fc.constant('{"root":{"type":"root","children":[{"type":"paragraph","children":[{"type":"text","text":"Test content"}]}]}}'),
					// Invalid JSON
					fc.string().filter(s => {
						try {
							JSON.parse(s)
