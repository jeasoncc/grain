...
import { getContentsByNodeIds, getNodesByWorkspace, getWorkspaceById } from "@/io/api"
 ...
 */
export function exportAsMarkdown(workspaceId: string): TE.TaskEither<AppError, string> {
	...
	return pipe(
		...
		TE.bind("contentMap", ({ nodes }) => {
			...
			return pipe(
				...
				TE.map((contents) => new Map(contents.map((c) => [c.nodeId, c.content]))),
			)
		}),
		// 转换为 Markdown
 ...
 */
export async function exportAsMarkdownAsync(workspaceId: string): Promise<string> {
