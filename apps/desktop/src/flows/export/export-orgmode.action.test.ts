/**
 * @file export-orgmode.action.test.ts
 * @description Org-mode 导出 Action 的单元测试
 *
 * 测试覆盖：
 * - 节点内容导出
 * - 直接内容导出
...
import { exportContentToOrgmode, exportNodeToOrgmode } from "./export-orgmode.flow"

// ============================================================================
// Unit Tests
// ============================================================================

describe("exportNodeToOrgmode", () => {
	beforeEach(() => {
		vi.clearAllMocks()
	})

	it("should export node content to Org-mode format", async () => {
		...
		const result = await runTE(exportNodeToOrgmode({ nodeId: testNode.id }))
	...
	it("should include title in export when option is set", async () => {
		...
		expect(E.isRight(result)).toBe(true)
		if (E.isRight(result)) {
			expect(result.right.content).toContain("#+TITLE: My Document")
		}
	})
...
describe("exportContentToOrgmode", () => {
	it("should export content directly to Org-mode format", () => {
		...
		const result = exportContentToOrgmode(content)
	...
	it("should return Left for invalid content", () => {
		const result = exportContentToOrgmode("invalid json")
	...
	it("should return Left for empty content", () => {
		const result = exportContentToOrgmode("")
