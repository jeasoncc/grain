...
`
		...
		expect(mockAddNode).toHaveBeenCalledWith("workspace-1", "Custom Title", expect.any(Object))
	})
...
`
	...
	it("should use default title when no title is available", async () => {
		...
		expect(mockAddNode).toHaveBeenCalledWith("workspace-1", "导入的文档", expect.any(Object))
	})
...
`
		...
		expect(E.isRight(result)).toBe(true)
		expect(mockGetNextOrder).toHaveBeenCalledWith("parent-node-id", "workspace-1")
