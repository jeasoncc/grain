 ...
 */
export function convertTagNode(node: LexicalTagNode, options: OrgmodeExportOptions = {}): string {
 ...
 * @returns Org-mode 字符串
 */
export function convertListNode(node: LexicalListNode, options: OrgmodeExportOptions = {}): string {
...
export function convertRootChild(
	...
	switch (node.type) {
		...
		case "heading": {
			const linesWithBlankBefore =
				opts.blankLineBeforeHeading && prevNodeType ? [...lines, ""] : lines
			const linesWithHeading = [...linesWithBlankBefore, convertHeadingNode(node, opts)]
			const finalLines = opts.blankLineAfterHeading ? [...linesWithHeading, ""] : linesWithHeading
			return finalLines.join("\n")
		}
		case "list": {
			const linesWithBlankBefore =
				opts.blankLineBeforeList && prevNodeType && prevNodeType !== "list" ? [...lines, ""] : lines
			const linesWithList = [...linesWithBlankBefore, convertListNode(node, opts)]
			const finalLines = opts.blankLineAfterList ? [...linesWithList, ""] : linesWithList
			return finalLines.join("\n")
		}
		default:
			// 处理未知节点类型
...
export function generateOrgProperties(options: OrgmodeExportOptions): string {
	...
	const linesWithTitle = options.title ? [...lines, `#+TITLE: ${options.title}`] : lines
	...
	const result = finalLines.length > 0 ? [...finalLines, ""] : finalLines
...
export function convertDocumentToOrgmode(
	...
	const linesWithProperties =
		opts.includeProperties || opts.includeTitle
			? (() => {
