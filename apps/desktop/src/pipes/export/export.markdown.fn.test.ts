...
describe("export.markdown.fn", () => {
	...
	describe("convertInlineNodes", () => {
		it("should convert mixed inline nodes", () => {
			...
			expect(convertInlineNodes(nodes, { tagFormat: "hash" })).toBe(
				"Hello #world",
			);
		});
	});
			...
			const contents = [
				{
					...
					content: JSON.stringify(
						createDocument([createParagraphNode([createTextNode("Doc 1")])]),
					),
					title: "First",
				},
				{
					id: "2",
					content: JSON.stringify(
						createDocument([createParagraphNode([createTextNode("Doc 2")])]),
					),
					title: "Second",
				},
			];
