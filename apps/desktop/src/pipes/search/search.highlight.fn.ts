...
export const extractTextFromLexical = (node: unknown): string => {
	...
	if (Array.isArray(n.children)) {
		return pipe(n.children, A.map(extractTextFromLexical), (texts) => texts.join(" "))
	}
	...
	return pipe(
		...
		O.match(
			...
			(index) => {
				...
				const end = Math.min(content.length, index + query.length + halfContext)
 ...
 * @returns 数值分数（越高越相关）
 */
export const calculateSimpleScore = (title: string, content: string, query: string): number => {
