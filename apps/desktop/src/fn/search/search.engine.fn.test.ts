...
describe("SearchEngine", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(getNodesByWorkspace).mockImplementation((workspaceId: string) => {
			return () =>
				Promise.resolve(
					E.right(mockNodes.filter((n) => n.workspace === workspaceId)),
				);
		});
		vi.mocked(getContentsByNodeIds).mockReturnValue(() =>
			Promise.resolve(E.right(mockContents)),
		);
		vi.mocked(getWorkspaceById).mockImplementation((id: string) => {
			return () =>
				Promise.resolve(
					E.right(mockWorkspaces[id as keyof typeof mockWorkspaces]),
				);
		});
	});
	...
	describe("buildIndex", () => {
		it("应该成功构建索引", async () => {
			...
			expect(getContentsByNodeIds).toHaveBeenCalledWith([
				"node-1",
				"node-2",
				"node-3",
			]);
	...
	describe("simpleSearch", () => {
		...
		it("应该不区分大小写", async () => {
			...
			vi.mocked(getAllNodes).mockReturnValue(() =>
				Promise.resolve(E.right(nodesWithEnglish)),
			);
	...
	describe("边界情况", () => {
		...
		it("应该处理没有标签的节点", async () => {
			...
			vi.mocked(getAllNodes).mockReturnValue(() =>
				Promise.resolve(E.right(nodesWithoutTags)),
			);
