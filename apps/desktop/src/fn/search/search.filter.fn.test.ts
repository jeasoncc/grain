 ...
 */
function createSearchableItem(
	overrides: Partial<SearchableItem> = {},
): SearchableItem {
	return {
		id: overrides.id ?? "test-id",
		title: overrides.title ?? "Test Title",
...
describe("filterByType", () => {
	it("should return all results when types is undefined", () => {
		const results = [
			createSearchResult({ type: "node" }),
			createSearchResult({ type: "project" }),
		];
...
describe("filterByMinScore", () => {
	it("should return all results when minScore is undefined", () => {
		const results = [
			createSearchResult({ score: 10 }),
			createSearchResult({ score: 50 }),
		];
