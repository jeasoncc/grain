...
use crate::services::{BackupInfo, BackupService};
...
pub async fn create_backup(config: State<'_, AppConfig>) -> Result<BackupInfo, String> {
    BackupService::create_backup(&config).map_err(|e| e.to_string())
}
...
pub async fn restore_backup(
    ...
    BackupService::restore_backup(&config, &path).map_err(|e| e.to_string())
}

/// 列出所有备份
#[tauri::command]
pub async fn list_backups(config: State<'_, AppConfig>) -> Result<Vec<BackupInfo>, String> {
    BackupService::list_backups(&config).map_err(|e| e.to_string())
}
...
pub async fn cleanup_old_backups(
    ...
    BackupService::cleanup_old_backups(&config, keep_count).map_err(|e| e.to_string())
}
