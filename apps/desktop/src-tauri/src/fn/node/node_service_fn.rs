...
mod tests {
    ...
    async fn test_delete_node_recursive() {
        ...
        // 验证所有节点都已删除
        assert!(node_db_fn::find_by_id(&db, "parent").await.unwrap().is_none());
        assert!(node_db_fn::find_by_id(&db, "child-1").await.unwrap().is_none());
        assert!(node_db_fn::find_by_id(&db, "child-2").await.unwrap().is_none());
    }
