...
/// 查询子节点
pub async fn find_children(db: &DatabaseConnection, parent_id: &str) -> AppResult<Vec<node::Model>> {
    let nodes = Node::find()
...
/// 获取节点的所有后代（递归）
pub async fn find_descendants(db: &DatabaseConnection, node_id: &str) -> AppResult<Vec<node::Model>> {
...
pub async fn reorder_nodes(db: &DatabaseConnection, node_ids: Vec<String>) -> AppResult<()> {
    ...
    for (index, node_id) in node_ids.iter().enumerate() {
        let existing = find_by_id(db, node_id).await?.ok_or_else(|| AppError::NotFound {
            entity: "Node".to_string(),
...
mod tests {
    ...
    async fn test_find_by_id_found() {
        ...
        let result = find_by_id(&db, "node-1").await;

        assert!(result.is_ok());
        let node = result.unwrap();
        assert!(node.is_some());
        assert_eq!(node.unwrap().title, "测试");
    }
    ...
    async fn test_find_root_nodes() {
        ...
        let result = find_root_nodes(&db, &workspace_id).await;
    ...
    async fn test_find_by_type() {
        ...
        let files = find_by_type(&db, &workspace_id, NodeType::File).await.unwrap();
        let folders = find_by_type(&db, &workspace_id, NodeType::Folder).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_next_sort_order() {
        let db = setup_test_db().await;
        let workspace_id = create_test_workspace(&db).await;

        // 第一个节点应该是 0
        let order1 = get_next_sort_order(&db, &workspace_id, None).await.unwrap();
        assert_eq!(order1, 0);
        ...
        // 第二个节点应该是 1
        let order2 = get_next_sort_order(&db, &workspace_id, None).await.unwrap();
        assert_eq!(order2, 1);
    }
    ...
    async fn test_move_node() {
        ...
        // 移动到 folder-2
        let result = move_node(&db, "file-1", Some("folder-2".to_string()), 0).await;

        assert!(result.is_ok());
        let node = result.unwrap();
        assert_eq!(node.parent_id, Some("folder-2".to_string()));
    }
