...
import { buildComprehensiveErrorMessage, getImmutableArrayAlternative } from '../../utils/message-builder.js';
 ...
 * 数组变异方法的错误消息生成器
 */
const getArrayMutationMessage = (method: string) => buildComprehensiveErrorMessage({
    ...
    return {
      CallExpression(node: TSESTree.CallExpression) {
        if (
          ...
          if (ARRAY_MUTATION_METHODS.includes(methodName as typeof ARRAY_MUTATION_METHODS[number])) {
            const messageIdMap: Record<string, 'noArrayPush' | 'noArrayPop' | 'noArrayShift' | 'noArrayUnshift' | 'noArraySplice' | 'noArrayReverse' | 'noArrayFill' | 'noArrayCopyWithin' | 'noArraySort'> = {
              push: 'noArrayPush',
              pop: 'noArrayPop',
              shift: 'noArrayShift',
