    ...
    return {
      CallExpression(node: TSESTree.CallExpression) {
        if (
          ...
          switch (methodName) {
            case 'push':
              context.report({ node, messageId: 'noArrayPush' });
              break;
            case 'sort':
              context.report({ node, messageId: 'noArraySort' });
              break;
          }
        }
      },
