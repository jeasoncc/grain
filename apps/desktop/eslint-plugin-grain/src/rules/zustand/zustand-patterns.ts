	...
	create(context) {
		...
		return {
			...
			'CallExpression[callee.name="create"] > ArrowFunctionExpression'(
				...
				const parent = node.parent
				if (parent && parent.type === "CallExpression" && parent.callee.type === "CallExpression") {
					// 检查是否有 immer 调用
					const outerCall = parent.callee
					if (outerCall.callee.type === "Identifier" && outerCall.callee.name === "immer") {
						// 使用了 immer，允许变异
						return
					}
				}
			...
			"Program:exit"() {
				...
				if (storeArg.type === "ArrowFunctionExpression") {
					...
					} else if (storeArg.body.type === "BlockStatement" && storeArg.body.body.length > 0) {
						const returnStmt = storeArg.body.body.find((s) => s.type === "ReturnStatement") as
