...
describe('Property 1: Error Handling Pattern Detection', () => {
  ...
  const identifierArb = fc.stringMatching(/^[a-zA-Z_][a-zA-Z0-9_]{0,10}$/)
    .filter(s => !['try', 'catch', 'throw', 'if', 'else', 'for', 'while', 'function', 'const', 'let', 'var', 'return', 'new', 'Promise'].includes(s));
  ...
  describe('Promise methods detection', () => {
    ...
    it('should detect Promise.all() calls', () => {
      fc.assert(
        fc.property(
          fc.array(identifierArb, { minLength: 1, maxLength: 5 }),
          (promises) => {
...
describe('Property 2: Immutability Enforcement', () => {
  ...
  const identifierArb = fc.stringMatching(/^[a-zA-Z_][a-zA-Z0-9_]{0,10}$/)
    .filter(s => !['push', 'pop', 'shift', 'unshift', 'splice', 'sort', 'reverse', 'fill', 'forEach', 'const', 'let', 'var', 'function', 'return', 'if', 'else'].includes(s));
    ...
    it('should detect all array mutation methods', () => {
      fc.assert(
        fc.property(arrayMutationMethodArb, identifierArb, (method, arrayName) => {
          ...
          const args = method === 'push' || method === 'unshift' ? '1' :
                       method === 'splice' ? '0, 1' :
                       method === 'fill' ? '0' : '';
      ...
      fc.assert(
        fc.property(identifierArb, (arrayName) => {
          ...
          expect(errors.filter(e => 
            e.ruleId === 'grain/no-mutation' || 
            e.ruleId === 'grain/no-object-mutation'
          ).length).toBe(0);
        }),
        { numRuns: 100 }
      );
