...
describe('Property 9: Code Complexity Metrics Enforcement', () => {
  ...
  const identifierArb = fc.stringMatching(/^[a-zA-Z_][a-zA-Z0-9_]{0,10}$/)
    .filter(s => !['function', 'const', 'let', 'var', 'return', 'if', 'else', 'for', 'while'].includes(s));
      ...
      fc.assert(
        fc.property(
          ...
          (lines, funcName) => {
            ...
            const bodyLines = Array(lines - 2).fill('  console.log("line");').join('\n');
    ...
    it('should not report functions within limit', () => {
      fc.assert(
        fc.property(
          ...
          identifierArb,
          (lines, funcName) => {
            const bodyLines = Array(Math.max(1, lines - 2)).fill('  console.log("line");').join('\n');
            const code = `function ${funcName}() {\n${bodyLines}\n}`;
