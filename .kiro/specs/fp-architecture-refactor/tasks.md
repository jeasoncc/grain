...
- [x] 118. 创建记账模板生成函数
  - 创建 `fn/ledger/ledger.template.fn.ts`
  - 生成记账 Lexical JSON 模板
  - 模板结构：
    - 标签行：#[ledger] #[日期]
    - H2 标题：日期 - Ledger
    - H2 Income：收入列表（使用 `-` 列表项）
    - H2 Expenses：支出列表（使用 `-` 列表项）
    - H2 Summary：汇总列表
    - H2 Notes：备注区域
  - 创建测试文件 `ledger.template.fn.test.ts`（12 个测试通过）
  - ✅ 已完成
...
- [x] 119. 创建记账配置
  ...
  - 配置：
    ...
    - fileType: "file"
    - tag: "ledger"
    - 文件夹结构：Ledger > year-YYYY > month-MM-MonthName > ledger-YYYY-MM-DD
  - ✅ 已完成
...
- [x] 120. 创建记账 Action
  ...
  - 创建测试文件 `create-ledger.action.test.ts`（10 个测试通过）
  - ✅ 已完成
