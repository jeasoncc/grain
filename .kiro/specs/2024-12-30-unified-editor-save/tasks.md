...
- [x] 1. TabType 复用 NodeType
  - [x] 1.1 修改 editor-tab.interface.ts，将 TabType 改为 `Exclude<NodeType, "folder">`
...
- [x] 2. 迁移 ExcalidrawEditorContainer
  - [x] 2.1 引入 useEditorSave hook 替换自定义保存逻辑
...
- [x] 5. 验证 DiagramEditor
  - [x] 5.1 确认 DiagramEditor 已正确使用 useEditorSave
    - 检查 hook 配置是否正确
    - 验证 tabId 传递正确
    - _Requirements: 5.1_
  - [x] 5.2 验证 onSaveSuccess 触发预览渲染
    - 测试保存后预览自动更新
