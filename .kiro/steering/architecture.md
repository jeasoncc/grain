...
所有编辑器组件（Lexical、Excalidraw、Diagram、Code）共享统一的保存机制：
...
│                          ▼                                     │
│              ┌─────────────────────┐                          │
│              │   useUnifiedSave    │  ← 唯一的保存 Hook        │
│              └──────────┬──────────┘                          │
│                         │                                      │
│                         ▼                                      │
│              ┌─────────────────────┐                          │
│              │ UnifiedSaveService  │  ← 唯一的保存服务         │
│              │   saveContent()     │  ← 唯一的保存入口         │
│              └──────────┬──────────┘                          │
│                         │                                      │
│         ┌───────────────┼───────────────┐                     │
...
| 组件 | 职责 | 位置 |
|------|------|------|
| `useUnifiedSave` | React Hook，提供保存接口 | `hooks/use-unified-save.ts` |
| `UnifiedSaveService` | 保存服务，处理实际保存逻辑 | `fn/save/unified-save.service.ts` |
| `saveContent()` | 唯一的保存入口函数 | `UnifiedSaveService` |

### 设计原则

- **单一入口**：所有编辑器通过 `useUnifiedSave` Hook 保存
- **统一状态**：保存状态（dirty、saving）集中管理
- **解耦编辑器**：编辑器组件不直接依赖保存逻辑
